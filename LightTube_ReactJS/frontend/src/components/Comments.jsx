...
const Comments = (props) => {
  ...
  useEffect(() => {
    const fetchComment = async() =>{
      try {
        const res = await axios.get(`http://localhost:3000/api/comment/${videoId}`)
        setComment(res.data)
      } catch (error) {
        console.log(error);
      }
  ...
  const handleComment = async(e) =>{
    ...
    try {
      const commentData = await axios.post(`http://localhost:3000/api/comment`, headers,{withCredentials : true})
      setCommentInput("")
