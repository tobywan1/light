...
const Comment = ({comment}) => {
  ...
  useEffect( () => {
    const fetchComment =async()=> {
      const channelResp = await axios.get(`http://localhost:3000/api/user/find/${comment.userId}`)
  ...
  const handleDelete = async(e) => {
    ...
    e.preventDefault();
    try {
      await axios.delete(`http://localhost:3000/api/comment/${comment._id}`,{headers: {Authorization: 'Bearer'},withCredentials : true})
